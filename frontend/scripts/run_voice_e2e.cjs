...
(async () => {
  ...
  console.log('Registering test user A...');
  ...
  if (!regBody.success) {
    console.error('Registration failed A:', regBody);
    process.exit(1);
  }

  // create a second user to chat with
  const usernameB = `e2e_user_b_${ts}`;
  ...
  const regResB = await fetch('http://localhost:5000/api/auth/register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ username: usernameB, phoneNumber: phoneB, password })
  });
  const regBodyB = await regResB.json();
  if (!regBodyB.success) {
    console.error('Registration failed B:', regBodyB);
    process.exit(1);
  }

  // create a conversation between A and B using A's token
  console.log('Creating conversation between A and B...');
  const convRes = await fetch('http://localhost:5000/api/chat/conversation', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${regBody.token}` },
    body: JSON.stringify({ userId: regBodyB.user._id })
  });
  const convBody = await convRes.json();
  if (!convRes.ok) {
    console.error('Failed to create conversation:', convBody);
    process.exit(1);
  }
  ...
  await page.evaluate(([token, refresh, user, selectedConversationId]) => {
    localStorage.setItem('token', token);
    localStorage.setItem('refreshToken', refresh);
    localStorage.setItem('user', JSON.stringify(user));
    localStorage.setItem('selectedConversationId', selectedConversationId);
  }, [regBody.token, regBody.refreshToken, regBody.user, convBody.conversation?._id || convBody._id || convBody.data?._id]);
