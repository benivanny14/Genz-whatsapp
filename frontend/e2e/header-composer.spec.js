...
test('header + composer interactions after extraction', async ({ browser }) => {
  ...
  // 1) Header renders the peer name and the E2EE lock badge.
  await expect(pageA.getByRole('heading', { name: new RegExp(`${creds.b.username}.*Chat encrypted`) })).toBeVisible({ timeout: 15_000 });
  await expect(pageA.getByLabel('Chat encrypted end-to-end')).toBeVisible({ timeout: 10_000 });
