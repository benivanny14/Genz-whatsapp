...
test('block → send blocked → unblock → chat restored and send works', async ({ browser, request }) => {
  ...
  const block = await request.post(`/api/chat/users/${userBId}/block`, {
    headers: { Authorization: `Bearer ${bodyA.token}` }
  });
  ...
  const blockedSend = await request.post('/api/chat/messages', {
  ...
  const unblock = await request.delete(`/api/chat/users/${userBId}/block`, {
    headers: { Authorization: `Bearer ${bodyA.token}` }
  });
  expect(unblock.status()).toBe(200);

  // Confirm the block is really gone server-side before reloading.
  const blockedAfter = await request.get('/api/auth/blocked', {
    headers: { Authorization: `Bearer ${bodyA.token}` }
  });
  ...
  const afterSend = await request.post('/api/chat/messages', {
    headers: { Authorization: `Bearer ${bodyB.token}`, 'Content-Type': 'application/json' },
    data: {
