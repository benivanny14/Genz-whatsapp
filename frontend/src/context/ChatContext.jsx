...
import { getEffectiveNotificationPreferences } from '../utils/notificationPreferences';
...
export const ChatProvider = ({ children }) => {
  ...
  const setMods = useCallback((updater) => {
    setModsState(prev => {
      ...
      try { 
        localStorage.setItem(getGENZModsKey(currentUserId), JSON.stringify(newMods)); 
        const settingsKey = getGENZSettingsKey(currentUserId);
        const existing = JSON.parse(localStorage.getItem(settingsKey) || '{}');
        existing.mods = newMods;
        localStorage.setItem(settingsKey, JSON.stringify(existing));
        // Force refresh for other components that might be listening
    ...
    try {
      ...
      socket.on('message:received', async (msg) => {
        ...
        try {
          ...
          const prefs = getEffectiveNotificationPreferences('message');
          if (!isMuted && !prefs.isDnd && prefs.sound) playMessageSound();
        } catch (_) {}
        try {
...
const prefs = getEffectiveNotificationPreferences('message');
              if (!isMuted && !isDNDModeRef.current && prefs.enabled) {
                ...
                const effectivePreview = prefs.showPreview ? preview : 'New message';
                notifyNewMessage(senderName, effectivePreview, incoming.conversationId);
      ...
      socket.on('group_call:incoming', (data) => {
        ...
        const prefs = getEffectiveNotificationPreferences('call');
        if (prefs.enabled) notifyIncomingCall(data.callerName, data.callType);
      });
      ...
      socket.on('call:incoming', ({ callerId, callType, conversationId, offer, callerName: socketCallerName, callerPicture: socketCallerPicture, callerSocketId } = {}) => {
        ...
        if (prefs.enabled) notifyIncomingCall(callerName, callType);
      });
  ...
  useEffect(() => {
    ...
    const targetConversationId = params.get('conversationId') || params.get('chatId') || params.get('chat');
  ...
  const addContact = async (phone, savedName) => {
    try {
      ...
      // Fallback: Store contact locally if backend endpoint fails
      console.warn('[ChatContext] Backend contact add failed, using local fallback:', error);
      try {
        const localContact = {
          _id: `local-${Date.now()}`,
          phone,
        ...
        return { success: true, message: 'Contact added locally (backend unavailable)' };
      } catch (localError) {
