...
const getIceServers = () => {
  ...
  const meteredUsername = import.meta.env.VITE_METERED_TURN_USERNAME;
  const meteredPassword = import.meta.env.VITE_METERED_TURN_PASSWORD;
  ...
  // servers so calls still work across restrictive networks. This branch
  // only matters if the backend /webrtc/config endpoint is unreachable —
  // normally the backend's own (now-fixed) fallback is used instead.
  if (!(meteredUsername && meteredPassword)) {
    const turnUrl = import.meta.env.VITE_TURN_SERVER_URL;
    const turnUsername = import.meta.env.VITE_TURN_USERNAME;
    const turnCredential = import.meta.env.VITE_TURN_CREDENTIAL;
