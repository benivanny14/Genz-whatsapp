...
function App() {
  ...
  const [appLockSettings, setAppLockSettings] = useState(null);
  ...
  useEffect(() => {
    ...
    const handleCallDeclineEvent = (event) => {
      handleNotificationAction({ ...(event.detail || {}), type: 'CALL_DECLINE' });
    };

    const handleNativeIncomingCall = (event) => {
      if (event.detail) setActiveCall(event.detail);
    };

    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('message', handleServiceWorkerMessage);
    }
    window.addEventListener('call-decline', handleCallDeclineEvent);
    window.addEventListener('native-incoming-call', handleNativeIncomingCall);
    ...
    return () => {
      ...
      window.removeEventListener('call-decline', handleCallDeclineEvent);
      window.removeEventListener('native-incoming-call', handleNativeIncomingCall);
    };
  }, [setActiveCall]);

  // --- App Lock ---
  useEffect(() => {
    const loadAppLockSettings = () => {
      try {
        const storedLockSettings = localStorage.getItem('genz_app_lock');
        if (storedLockSettings) {
          const parsed = JSON.parse(storedLockSettings);
          const nextSettings = parsed && typeof parsed === 'object' ? parsed : null;
          setAppLockSettings(nextSettings);
          if (nextSettings?.appLockEnabled && !showAppLock) {
            setShowAppLock(true);
          }
        } else {
          setAppLockSettings(null);
        }
      } catch (e) {
        console.error('Failed to parse app lock settings', e);
      }
    };

    loadAppLockSettings();

    window.addEventListener('app-lock-changed', loadAppLockSettings);
    return () => window.removeEventListener('app-lock-changed', loadAppLockSettings);
  }, [showAppLock]);

  const handleAppLockToggle = (settings) => {
    setAppLockSettings(settings);
    localStorage.setItem('genz_app_lock', JSON.stringify(settings));
  ...
  return (
    <ErrorBoundary>
      ...
      {showAppLock && (
        <AppLock
          isEnabled={appLockSettings?.appLockEnabled || false}
      ...
      <Suspense fallback={<PageLoader />}>
        <Routes>
          ...
          */}
          <Route
            path="/system-control-x7k9/login"
            element={
              <AdminAuthProvider>
                <AdminLogin />
              </AdminAuthProvider>
          ...
          <Route path="/community" element={<ProtectedRoute><CommunityManager /></ProtectedRoute>} />
