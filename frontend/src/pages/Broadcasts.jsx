...
const Broadcasts = () => {
  ...
  const normalizeBroadcast = (broadcast) => ({
    ...
    recipientCount: broadcast.recipientCount ?? broadcast.recipients?.length ?? 0,
    messageCount: broadcast.messageCount ?? 0,
    deliveredCount: broadcast.deliveredCount ?? 0,
    lastSent: broadcast.lastSent || broadcast.sentAt || broadcast.timestamp || null
  });
  ...
  const fetchBroadcasts = async () => {
    try {
      ...
      const normalized = (data.broadcasts || []).map(normalizeBroadcast);
