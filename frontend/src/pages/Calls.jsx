...
import { Phone, Video, PhoneIncoming, PhoneMissed, PhoneOutgoing, Search, ArrowLeft, Clock, UserPlus, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#0d1b2a] flex flex-col">
      <div className="bg-gradient-to-r from-[#1f2c34] to-[#0d1b2a] px-4 py-3 flex items-center gap-3 sticky top-0 z-50 border-b border-white/5">
        <button type="button" onClick={() => navigate(-1)} className="p-2 rounded-xl hover:bg-white/10 transition-all text-white/70">
          <ArrowLeft size={20} />
        </button>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map((log) => (
              <div
                key={log._id}
                className="bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl p-4 flex items-center gap-3 transition-all group"
              >
