...
export const chatAPI = {
  ...
  removeAdmin: (id, userId) => safe({})(() => api.delete(`/chat/groups/${id}/admins/${userId}`)),
  leaveGroup: (id) => safe({})(() => api.delete(`/chat/groups/${id}/leave`)),
  // Group member management
  banMember: (groupId, userId, reason = '') => safe({})(() => api.post(`/chat/groups/${groupId}/ban/${userId}`, { reason })),
  unbanMember: (groupId, userId) => safe({})(() => api.delete(`/chat/groups/${groupId}/ban/${userId}`)),
  getBannedMembers: (groupId) => safe({ bannedMembers: [] })(() => api.get(`/chat/groups/${groupId}/banned`)),
  transferOwnership: (groupId, newOwnerId) => safe({})(() => api.put(`/chat/groups/${groupId}/transfer-ownership`, { newOwnerId })),
  getPendingJoinRequests: (groupId) => safe({ requests: [] })(() => api.get(`/chat/groups/${groupId}/pending-requests`)),
  approveJoinRequest: (groupId, userId) => safe({})(() => api.post(`/chat/groups/${groupId}/pending-requests/${userId}/approve`)),
  rejectJoinRequest: (groupId, userId) => safe({})(() => api.post(`/chat/groups/${groupId}/pending-requests/${userId}/reject`)),
  updateAntiSpam: (groupId, settings) => safe({})(() => api.put(`/chat/groups/${groupId}/antispam`, settings)),
  updateJoinApproval: (groupId, requireApproval) => safe({})(() => api.put(`/chat/groups/${groupId}/join-approval`, { requireApproval })),
  getGroupQR: (groupId) => safe({})(() => api.get(`/chat/groups/${groupId}/qr`)),
  // Group events
  getGroupEvents: (groupId) => safe({ events: [] })(() => api.get(`/chat/groups/${groupId}/events`)),
  createGroupEvent: (groupId, data) => safe({})(() => api.post(`/chat/groups/${groupId}/events`, data)),
  rsvpGroupEvent: (groupId, eventId, status) => safe({})(() => api.post(`/chat/groups/${groupId}/events/${eventId}/rsvp`, { status })),
