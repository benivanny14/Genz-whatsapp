...
class WebRTCService {
  ...
  async _createPeer() {
    ...
    pc.onicecandidate = (event) => {
      // Only send candidates if we have a remote description set
      if (event.candidate && this.socket && this.targetUserId && pc.remoteDescription) {
  ...
  async answerCall(offer, callType = 'audio', socket, callerId, conversationId = null) {
    this.socket = socket;
    ...
    try {
      ...
      socket.emit('webrtc:answer', { to: callerId, answer: answerPayload, conversationId });
      return this.localStream;
    } catch (err) {
