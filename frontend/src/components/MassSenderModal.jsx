...
const MassSenderModal = ({ onClose }) => {
  ...
  const contacts = useMemo(() => {
    ...
    conversations?.forEach(c => {
      c.participants?.forEach(p => {
        ...
        if (participantId && participantId !== currentUserId && !uniqueMap.has(participantId)) {
          uniqueMap.set(participantId, { ...p, _id: participantId, id: participantId });
        }
      });
    });
  ...
  }, [conversations, user?._id, user?.id]);
  ...
  const toggleAll = () => {
    if (selectedUsers.length === contacts.length) {
      ...
      setSelectedUsers((contacts || []).map(c => c._id).filter(Boolean));
    }
  };
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-primary-600/20 flex items-center justify-center text-primary-600 font-bold">
                    {(contact?.username || contact?.name || '?').charAt(0)}
                  </div>
                  <span className="text-dark-text text-sm">{contact?.username || contact?.name || 'Unknown User'}</span>
                </div>
