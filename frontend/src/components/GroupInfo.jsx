  ...
  <div className="fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-4" onClick={onClose}>
    ...
    >
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
        ...
        <button onClick={onClose} className="text-[#8696a0] hover:text-white p-1" aria-label="Close"><X size={18} /></button>
      </div>
...
const GroupInfo = ({ group, onClose, currentUserId }) => {
  ...
  const handleShareInviteLink = async () => {
    ...
    try {
      ...
      console.warn('[GroupInfo] Failed to copy invite link:', err);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#202c33] flex-shrink-0">
        <button onClick={onClose} className="text-[#aebac1] hover:text-white p-1" aria-label="Close group info">
          <ArrowLeft size={22} />
        </button>
        <h2 className="text-white font-semibold text-base flex-1">Group Info</h2>
        {isAdmin && !editMode && (
          <button onClick={() => setEditMode(true)} className="text-[#aebac1] hover:text-white p-1" aria-label="Edit group info">
            <Edit2 size={18} />
          </button>
        )}
      </div>
            ...
            {activeTab === 'info' && (
              <div>
                ...
                <div className="bg-[#111b21] mt-2 py-1">
                  <SectionRow icon={LinkIcon} label="Share invite link" onClick={handleShareInviteLink}
                      ...
                      {!isSelf && isAdmin && (
                        <div className="flex items-center gap-1">
                          {!isMemberAdmin ? (
                            ...
                            <button onClick={() => handleRemoveAdmin(memberId)} className="p-1.5 text-yellow-400 hover:bg-yellow-400/20 rounded" title="Remove Admin" aria-label="Remove admin">
                              <Shield size={15} />
                            </button>
                          )}
                          <button onClick={() => handleRemoveMember(memberId)} className="p-1.5 text-orange-400 hover:bg-orange-400/20 rounded" title="Remove" aria-label="Remove member">
                            <UserMinus size={15} />
                          </button>
                          <button onClick={() => handleBanMember(memberId, member?.username)} className="p-1.5 text-red-400 hover:bg-red-400/20 rounded" title="Ban" aria-label="Ban member">
                            <Ban size={15} />
                          </button>
                        </div>
                      )}
