...
const ContactInfo = ({
  ...
  const [chatLanguage, setChatLanguage] = useState(conversation?.language || '');
  const [contactOverride, setContactOverride] = useState({ showLastSeen: true, showReadReceipts: true });
  ...
  /* Log a profile view (Who Viewed My Profile) — never for our own profile */
  useEffect(() => {
    const viewedUserId = contact?._id || contact?.id;
    if (viewedUserId && viewedUserId !== currentUserId) {
      profileViewService.recordView(viewedUserId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contact?._id, contact?.id]);

  /* Load this contact's per-chat privacy override ("Custom Tick & Last Seen for Specific Contact") */
  useEffect(() => {
    const contactId = contact?._id || contact?.id;
    if (!contactId || conversation?.isGroup) return;
    (async () => {
      try {
        const res = await authFetch(apiUrl('/api/genz-mods/contact-privacy'));
        if (!res.ok) return;
        const data = await res.json();
        const existing = data.overrides?.[contactId];
        if (existing) {
          setContactOverride({
            showLastSeen: existing.showLastSeen !== false,
            showReadReceipts: existing.showReadReceipts !== false
          });
        }
      } catch (_) {
        // non-critical
  ...
  }, [contact?._id, contact?.id, conversation?.isGroup]);

  const updateContactOverride = async (patch) => {
    const contactId = contact?._id || contact?.id;
    if (!contactId) return;
    const next = { ...contactOverride, ...patch };
    setContactOverride(next);
    try {
      await authFetch(apiUrl(`/api/genz-mods/contact-privacy/${contactId}`), {
        method: 'PUT',
  ...
  const updateChatLanguage = async (lang) => {
    setChatLanguage(lang);
    setShowLanguagePicker(false);
    const convId = conversation?._id;
    if (!convId) return;
    try {
      await chatService.setConversationLanguage(convId, lang);
    } catch (_) {
      // non-critical
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain custom-scrollbar">
          ...
          {/* ─── 7b. Language Per Chat ─── */}
          <div className="bg-[#202c33]">
            <SectionRow
              icon={Globe}
              iconColor="text-white/50"
              label="Chat language"
              value={chatLanguage ? chatLanguage.toUpperCase() : 'Default'}
          ...
          {!conversation?.isGroup && (
            <>
              {/* ─── 7c. Custom Tick & Last Seen for this contact ─── */}
              <div className="bg-[#202c33]">
                <SectionRow
                  icon={contactOverride.showLastSeen ? Eye : EyeOff}
                  iconColor="text-white/50"
                  label="Custom privacy for this contact"
                  value={showCustomPrivacy ? 'Hide' : 'Show'}
                  onClick={() => setShowCustomPrivacy((v) => !v)}
                />
                {showCustomPrivacy && (
                  <div className="px-4 pb-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-white/70">Mruhusu aone Last Seen yangu</span>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={contactOverride.showLastSeen}
                          onChange={(e) => updateContactOverride({ showLastSeen: e.target.checked })}
                          className="sr-only peer"
                        />
                        <div className="w-10 h-5 bg-white/10 peer-checked:bg-[#00a884] rounded-full peer after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5" />
                      </label>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-white/70">Mruhusu aone Blue Ticks zangu</span>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={contactOverride.showReadReceipts}
                          onChange={(e) => updateContactOverride({ showReadReceipts: e.target.checked })}
                          className="sr-only peer"
                        />
                        <div className="w-10 h-5 bg-white/10 peer-checked:bg-[#00a884] rounded-full peer after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5" />
                      </label>
      ...
      <AnimatePresence>
        ...
        {showLanguagePicker && (
          ...
          >
            <div
              className="bg-[#233138] w-full sm:w-80 sm:rounded-2xl rounded-t-2xl p-4"
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-white font-semibold mb-3 flex items-center gap-2">
                <Globe size={16} /> Chat Language
              </h3>
              <div className="space-y-1 max-h-72 overflow-y-auto">
                {[
                  { code: '', label: 'Default (device language)' },
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => updateChatLanguage(opt.code)}
                    className={`w-full text-left px-3 py-2.5 rounded-lg text-sm ${
                      chatLanguage === opt.code ? 'bg-[#00a884]/20 text-[#00a884]' : 'text-white/80 hover:bg-white/5'
                    }`}
                  >
                    {opt.label}
                  </button>
