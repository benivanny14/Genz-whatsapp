...
const StoryHighlights = ({ statuses = [], onSaveHighlight, compact = false }) => {
  // Previously this only wrote to localStorage — highlights were invisible to
  // everyone else and lost on a different device. Now backed by /api/story-highlights.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('Failed to load story highlights:', err);
      } finally {
  ...
  const createHighlight = async () => {
    ...
    try {
      const res = await authFetch(`${API_URL}/story-highlights/create`, {
        ...
        body: JSON.stringify({
          title: newName.trim(),
          statusIds: selectedStatuses.map((s) => s._id || s.id),
          coverImage: selectedStatuses[0]?.mediaUrl || null,
          category: String(selectedColor)
        })
      });
      const data = await res.json();
      if (data?.success && data.highlight) {
        const highlight = normalize(data.highlight, selectedStatuses);
        setHighlights((prev) => [...prev, highlight]);
        onSaveHighlight?.(highlight);
      }
    } catch (err) {
      console.warn('Failed to create story highlight:', err);
    }
    setNewName('');
    setSelectedStatuses([]);
    setShowCreate(false);
  ...
  const openHighlight = async (h) => {
    ...
    try {
      ...
      console.warn('Failed to load story highlight detail:', err);
    }
  };
      ...
      <div className="grid grid-cols-4 gap-3">
        {highlights.map((h) => (
          <div key={h.id} className="flex flex-col items-center gap-1 group relative">
            <button onClick={() => openHighlight(h)} className="highlight-ring w-16 h-16" style={{ background: h.color }}>
              <div className="highlight-ring-inner w-full h-full flex items-center justify-center overflow-hidden">
                {h.coverUrl ? (
                  <img src={h.coverUrl} alt={h.name} className="w-full h-full object-cover rounded-full" />
