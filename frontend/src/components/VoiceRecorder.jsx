  ...
  if (isPreviewMode) {
    return (
      <div className="flex-1 flex items-center justify-between gap-2 bg-dark-surface rounded-2xl p-2 px-4 border border-[#25D366]/35">
        <button type="button" onClick={cancelRecording} className="text-red-500 hover:text-red-600 transition-colors p-2" aria-label="Discard recording">
          <Trash2 size={20} />
        </button>
        ...
        <button
          ...
          title="View Once"
          aria-label="Toggle view-once for this voice note"
    ...
    return (
      <div className="flex-1 flex items-center gap-2 relative select-none touch-none">
        ...
        <div className="flex-1 flex items-center gap-2 px-2 py-1 bg-[#102029] border border-white/10 rounded-full relative overflow-hidden min-w-0 shadow-inner">
          ...
          {isLocked && (
            isPaused ? (
              <button type="button" onClick={resumeRecording} className="text-[#25D366] hover:bg-[#25D366]/10 p-2 rounded-full flex-shrink-0" aria-label="Voice message">
                <Mic size={18} />
              </button>
            ) : (
              <button type="button" onClick={pauseRecording} className="text-gray-400 hover:bg-white/5 p-2 rounded-full flex-shrink-0" aria-label="Pause">
                <Pause size={18} />
              </button>
            )
          )}
          ...
          {isLocked && (
            <button
              ...
              title="View Once" aria-label="View Once"
            >
    ...
    <div className="flex items-center gap-1 relative">
      <div className="relative">
        <button
          ...
          title="Voice effects / Voice changer" aria-label="Voice effects / Voice changer"
        >
          <Wand2 size={17} />
        </button>
        ...
        {showEffects && (
          <div className="absolute bottom-14 right-0 bg-[#0b141a] border border-white/12 rounded-2xl p-3 shadow-2xl z-[250] w-[280px] max-h-[52vh] overflow-y-auto">
            <p className="text-white text-xs font-bold mb-2 flex items-center gap-1">
              <Wand2 size={12} /> Voice changer (same as GENZ Settings)
            </p>
