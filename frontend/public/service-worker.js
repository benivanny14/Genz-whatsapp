...
self.addEventListener('fetch', (e) => {
  ...
  // Skip API calls, Socket.IO polling, APK downloads, and any non-GET requests
  if (
    request.method !== 'GET' ||
...
self.addEventListener('push', (e) => {
  ...
  // FIX: Handle missed/cancelled calls by closing the ringing notification
  if (notifType === 'missed_call' || notifType === 'cancel_call') {
    e.waitUntil(
      self.registration.getNotifications().then((notifications) => {
        // Close any active ringing notifications from this caller/callId
        notifications.forEach(n => {
          if (n.tag === payload.tag || n.tag === `call-${info.callId}` || n.data?.type === 'call') {
            n.close();
          }
        });
        
        // Show a standard, non-ringing missed call notification
        return self.registration.showNotification(title || 'Missed Call', {
          body: payload.body || info.text || `${info.callerName || 'Someone'} tried to call you`,
          icon: info.callerPicture || '/icons/icon-192x192.png',
          tag: `missed-${info.callId || Date.now()}`,
          data: { url: '/calls', type: 'missed_call' }
        });
      })
    );
    return;
...
self.addEventListener('notificationclick', (e) => {
  ...
  const targetUrl = data.conversationId ? `/?conversationId=${encodeURIComponent(data.conversationId)}` : (data.url || '/');
