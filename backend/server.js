...
if (!isCloudinaryConfigured()) {
  if (!fs.existsSync(uploadDir)) {
    ...
    logger.warn(
      "[WARNING] Using local storage. Configure Cloudinary for production to avoid data loss on redeploy.",
    );
  }
}
...
const ensureLocalUser = async () => {
  ...
  try {
    ...
    logger.warn("Could not ensure local fallback user:", {
...
const startScheduledMessageDispatcher = (ioInstance) => {
  ...
  scheduledMessageInterval = setInterval(async () => {
    try {
      ...
      for (const scheduledMsg of dueMessages) {
        try {
          ...
          await Conversation.findByIdAndUpdate(
            scheduledMsg.conversationId._id,
            {
          ...
          if (ioInstance) {
            const populatedMessage = await Message.findById(message._id)
              ...
              .populate("replyTo");
            ioInstance
              .to(scheduledMsg.conversationId._id.toString())
              .emit("message:received", populatedMessage);
          }

          logger.debug("[ScheduledMessage] Sent message", {
            messageId: message._id,
            conversationId: scheduledMsg.conversationId._id,
          });
        } catch (error) {
          logger.error("[ScheduledMessage] Failed to send message", {
            message: error.message,
          });
      ...
      logger.error("Scheduled message dispatcher error", {
        message: error.message,
      });
    }
  }, 30 * 1000);
  ...
  expiredMessageCleanupInterval = setInterval(async () => {
    try {
      ...
      if (expiredSelfDestruct.length > 0) {
        ...
        logger.debug("Deleted expired self-destruct messages", {
          count: expiredSelfDestruct.length,
        });
      }
      ...
      if (viewOnceResult.deletedCount > 0) {
        logger.debug("Deleted old view-once messages", {
          count: viewOnceResult.deletedCount,
        });
      }
      ...
      // TATIZO 4 FIX: Clean up Cloudinary media BEFORE deleting documents.
      await cleanupCloudinaryForQuery(
        hardDeleteBackstopQuery,
        "hard-delete-backstop",
      );
      const hardDeleteResult = await Message.deleteMany(
        hardDeleteBackstopQuery,
      );
      if (hardDeleteResult.deletedCount > 0) {
        logger.debug("Hard-deleted expired deletedForEveryone messages", {
          count: hardDeleteResult.deletedCount,
        });
      }
      ...
      if (Date.now() - lastOnlineHistoryPrune > 60 * 60 * 1000) {
        ...
        if (pruneResult.modifiedCount > 0) {
          logger.debug("Pruned old onlineHistory entries", {
            modified: pruneResult.modifiedCount,
          });
        }
      }
    } catch (error) {
...
const startBackgroundServices = async (ioInstance) => {
  ...
  if (!manualPaymentExpiryInterval) {
    manualPaymentExpiryInterval = setInterval(
      async () => {
        try {
          logger.info("Running manual payment expiry check");
          const result = await runExpiryCheck(ioInstance);
          logger.info("Manual payment expiry check completed", {
            processed: result,
          });
        } catch (error) {
          logger.error("Manual payment expiry check failed", {
            message: error.message,
          });
        }
      },
...
const trustProxyValue = String(process.env.TRUST_PROXY || "")
  .trim()
  .toLowerCase();
...
app.set(
  ...
  trustProxyEnabled
    ? true
    : process.env.TRUST_PROXY_IPS
      ? process.env.TRUST_PROXY_IPS.split(",")
      : false,
);
if (redisClient) {
  ...
  try {
    ...
    if (result && typeof result.then === "function") {
      return result.catch((err) => {
        logger.error("Rate limiter promise error", {
          message: err && err.message ? err.message : err,
        });
        return res
          .status(500)
          .json({ success: false, message: "Security check failed" });
      });
    }
    return result;
  } catch (err) {
    logger.error("Rate limiter sync error", {
      message: err && err.message ? err.message : err,
    });
    return res
      .status(500)
...
app.get("/api/health/ready", async (req, res) => {
  ...
  const ready =
    payload.services.mongo === "connected" ||
    process.env.NODE_ENV !== "production";
...
app.use("/uploads", secureUploads, (req, res) => {
  ...
  if (socketIdMatch) {
    ...
    logger.debug("Removed socket ID suffix from path", {
      requestId: req.id,
  ...
  if (
    ...
    logger.warn("Blocked unsafe upload path traversal attempt", {
      requestId: req.id,
      path: req.path,
    });
...
const {
  userRoutes: manualPaymentUserRoutes,
...
const ADMIN_BASE_PATH =
  process.env.ADMIN_BASE_PATH || "/api/system-gateway-x9k";
...
const handleUpload = async (req, res) => {
  try {
    ...
    if (isCloudinaryConfigured() && req.file.path) {
      const fileType =
        getFileType(req.file.originalname, req.file.mimetype) ||
...
if (fs.existsSync(frontendIndexPath)) {
  // version.json must NEVER be cached: the in-app update banner and
  // the login page compare the served values against the installed build, so
  // a day-old copy would hide new releases.
  ...
  app.get(
    "/version.json",
    sendNoCache(path.join(frontendDistPath, "version.json")),
  );
  ...
  const frontendPublicApk = path.resolve(
    __dirname,
    "../frontend/public/genz-whatsapp.apk",
  );
  app.get("/genz-whatsapp.apk", (req, res) => {
    const apkPath = fs.existsSync(
...
const gracefulShutdown = async (signal) => {
  ...
  const isFailureShutdown =
    signal === "uncaughtException" || signal === "startupFailure";
...
if (require.main === module) {
  server.listen(PORT, "0.0.0.0", () => {
    ...
    logger.info("TM Backend started", {
      ...
      fcmPush: fcmConfigured
        ? "enabled"
        : "disabled (set FIREBASE_* env vars to enable)",
    });
