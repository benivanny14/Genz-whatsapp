...
// SECURITY: the admin login gateway is mounted at an obscure, secret,
// configurable path — NOT at a guessable "/api/admin/auth". Set
// ADMIN_BASE_PATH in your .env to something only you know, e.g.
//   ADMIN_BASE_PATH=/api/sys-ctrl-7f3k9
// If it isn't set, a random-looking (but stable per boot) default is used
// and printed once to the server logs so you can find it.
const ADMIN_BASE_PATH = process.env.ADMIN_BASE_PATH || '/api/system-gateway-x9k2';
if (!process.env.ADMIN_BASE_PATH) {
  console.warn(`[SECURITY] ADMIN_BASE_PATH not set. Using default: ${ADMIN_BASE_PATH}`);
  console.warn('[SECURITY] Set ADMIN_BASE_PATH in your .env to a secret path only you know.');
}
...
app.use('/api/payment/manual', manualPaymentRoutes.userRoutes);
app.use('/api/admin/manual-payments', manualPaymentRoutes.adminRoutes);
...
if (fs.existsSync(frontendIndexPath)) {
  app.use(express.static(frontendDistPath, { maxAge: '1d', index: false, dotfiles: 'allow' }));
  app.get(/^\/(?!api\/|uploads\/|socket\.io|\.well-known\/).*/, (req, res, next) => {
    if (req.method !== 'GET' && req.method !== 'HEAD') return next();
    // Don't serve index.html for direct file requests (e.g. .apk downloads)
...
// Daily job: mark manual-payment subscriptions as Expired and lock premium
// access once their expiry date has passed (PART 9 / PART 10 of the manual
...
try {
  ...
  cron.schedule('0 * * * *', async () => {
    try {
      const count = await runExpiryCheck(io);
      if (count > 0) console.log(`[ManualPayment] Expired ${count} subscription(s)`);
    } catch (err) {
      console.error('[ManualPayment] Expiry check failed:', err.message);
    }
  });
} catch (err) {
  console.warn('[ManualPayment] node-cron not available, skipping expiry scheduler:', err.message);
}
