...
const setupSocket = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('user:join', async (userId) => {
      ...
      try {
        const user = await User.findByIdAndUpdate(userId, { isOnline: true, lastSeen: new Date() }, { new: true }).select('username profilePicture settings contacts isAdmin role');
        socket.data.username = user?.username || socket.data.username || 'Someone';
        socket.data.profilePicture = user?.profilePicture || socket.data.profilePicture || '';

        // Admins join a shared room so payment (and other) events can be broadcast to all admins.
    ...
    socket.on('call:accept', (data) => {
      ...
      if (callerId && conversationId) {
        activeCalls.markAccepted(callerId, conversationId);
      }
    ...
    socket.on('call:reject', async (data = {}) => {
      ...
      activeCalls.endCall(callerId || socket.userId, conversationId);
    });
    ...
    socket.on('call:end', async (data = {}) => {
      ...
      const activeSession = activeCalls.getCall(socket.userId, conversationId);
      ...
      // FIX: Stop ringing on the receiver's device by sending a missed call / cancel push notification
      if (targetUserId && activeSession && !activeSession.accepted) {
        const callerName = socket.data && socket.data.username ? socket.data.username : 'Someone';
        sendMissedCallNotification(targetUserId, {
          callId: conversationId, // Using conversationId as callId to match the frontend notification tag
          callerId: socket.userId,
          callerName: callerName,
          callerPicture: socket.data?.profilePicture || '',
          conversationId,
          callType: callType
        }).catch(err => console.error('[Push] Failed to send missed call notification:', err.message));
      }
...
try {
    ...
    socket.on('webrtc:answer', async (data) => {
      try {
        const { to, callerSocketId, answer, conversationId } = data;
        if (to && conversationId) {
          activeCalls.markAccepted(to, conversationId);
        }
          ...
          for (const session of endedSessions) {
            ...
            if (otherPartyId) {
              ...
              if (otherSocketId) {
                ...
                // FIX: Stop ringing on the receiver's device if the caller disconnected
                sendMissedCallNotification(otherPartyId, {
                  callId: session.conversationId,
                  callerId: disconnectedUserId,
                  callerName: 'Someone',
                  conversationId: session.conversationId,
                  callType: session.callType || 'audio'
                }).catch(err => console.error('[Push] Failed to send disconnect missed call notification:', err.message));
