    ...
    socket.on('message:delete', async (data) => {
      try {
        ...
        if (message.sender.toString() === socket.userId) {
          ...
          if (forEveryone) {
            // Delete for everyone: every participant should see it removed/replaced.
            io.to(message.conversationId.toString()).emit('message:deleted', {
              messageId,
              forEveryone: true
            });
          } else {
            // FIX: "delete for me" must only affect the deleter's own view.
            // This used to broadcast to the whole conversation room even
            // when forEveryone was false, so the message silently vanished
            // from the OTHER participant's chat too, even though they never
            // deleted anything. Only tell the requesting socket.
