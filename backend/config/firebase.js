...
const normalizeDataPayload = (data = {}, notification = {}) => {
  const output = {
    ...data,
...
const getAndroidMessagePriority = (priority) => (priority === 'normal' ? 'normal' : 'high');
const getAndroidNotificationPriority = (priority) => (priority === 'normal' ? 'default' : (priority || 'high'));
...
const sendNotification = async (token, notification, data = {}) => {
  ...
  try {
    const message = {
      ...
      data: normalizeDataPayload(data, notification),
      android: {
        priority: getAndroidMessagePriority(notification.priority),
        notification: {
          channelId: 'genz-whatsapp',
          priority: getAndroidNotificationPriority(notification.priority),
          sound: notification.sound || 'default'
        }
      },
      apns: {
