  ...
  try {
    ...
    const formatted = highlights.map(h => {
      ...
      return {
        id: String(obj._id),
        ...obj,
        coverUrl: obj.coverUrl || obj.statuses?.[0]?.mediaUrl || obj.statuses?.[0]?.content || ''
      };
    });
...
router.post('/create', async (req, res) => {
  try {
    ...
    const highlight = await StoryHighlight.create({
      userId: req.user._id,
      name: name.trim(),
      color: color || 'linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)',
      coverUrl: coverUrl || '',
      statuses: statusIds || []
    });

    const populated = await StoryHighlight.findById(highlight._id).populate('statuses');
...
router.get('/:id', async (req, res) => {
  try {
    const highlight = await StoryHighlight.findOne({ _id: req.params.id, userId: req.user._id })
      .populate('statuses');
...
router.delete('/:id', async (req, res) => {
  try {
    const deleted = await StoryHighlight.findOneAndDelete({ _id: req.params.id, userId: req.user._id });
    if (!deleted) {
      return res.status(404).json({ success: false, message: 'Highlight not found' });
    }
