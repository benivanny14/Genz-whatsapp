...
async function migrateStatusSchema(mongoUri) {
  try {
    ...
    console.log(`[Migration] Completed. Modified ${result.modifiedCount || result.nModified || 0} status documents.`);
  } catch (err) {
...
if (require.main === module) {
  const uri = process.env.MONGODB_URI || process.env.MONGO_URI || 'mongodb://localhost:27017/genz_whatsapp';
  migrateStatusSchema(uri).then(() => process.exit(0)).catch(() => process.exit(1));
}
