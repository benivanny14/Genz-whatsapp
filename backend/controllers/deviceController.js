...
exports.pairDevice = async (req, res) => {
  try {
    const currentUserId = getCurrentUserId(req);
    const pairingToken = req.body.pairingToken || req.body.code;
    const { deviceName, deviceType, platform, browser } = req.body;
    
    if (!pairingToken) {
      return res.status(400).json({ message: 'Pairing token is required' });
    }
    ...
    device.pairingToken = undefined; // Remove token after pairing
