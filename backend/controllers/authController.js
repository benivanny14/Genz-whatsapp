...
exports.login = async (req, res) => {
  try {
    ...
    // Auto-upgrade old password hash format to new faster format (N=4096)
    if (passwordMatch && user.passwordHash && user.passwordHash.split(':').length === 2) {
      await user.setPassword(password);
      // It will be saved by the user.save() call below
    }
    if (!passwordMatch) {
