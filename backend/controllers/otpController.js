...
exports.requestRegisterOTP = async (req, res) => {
  try {
    ...
    // Do NOT await, let it send in the background to prevent UI hanging
    smsService.sendOTP(normalizedPhone, otp).catch(err => console.error('[SMS Error]', err.message));
...
exports.verifyLoginOTP = async (req, res) => {
  try {
    ...
    // SECURITY FIX: this login path checked password + SMS OTP but never
    // checked twoFactorEnabled at all — anyone with 2FA turned on could
    // fully bypass it simply by using "Login with OTP" instead of the
    // password form. Enforce the same TOTP check the password-login path
    // already does.
    //
    // IMPORTANT ordering: OTPs are single-use (deleted from the store the
    // moment they're checked), so we must decide whether a 2FA code is
    // still needed BEFORE calling smsService.verifyOTP — otherwise the
    // follow-up request (same OTP + the 2FA code) would fail with "OTP not
    // found" because the first call already consumed it.
