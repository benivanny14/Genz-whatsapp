...
exports.getUserStatus = async (req, res) => {
  try {
    ...
    const isOnline = settings.alwaysOnline ? true : (settings.hideOnline || !settings.onlineStatusVisible ? false : user.isOnline);
    const lastSeen = (settings.hideLastSeen || settings.freezeLastSeen) ? null : user.lastSeen;

    res.status(200).json({
      success: true,
      userStatus: { isOnline, lastSeen }
    });
  } catch (error) {
...
// ---- Per-contact privacy overrides ("Custom Tick & Last Seen for Specific Contact") ----

// PUT /api/genz-mods/contact-privacy/:contactId  { showLastSeen?, showReadReceipts? }
exports.setContactPrivacyOverride = async (req, res) => {
  try {
    const user = await getUser(req, res);
    if (!user) return;
    ...
    const overrides = { ...(user.contactPrivacyOverrides || {}) };
    const existing = overrides[contactId] || {};
    overrides[contactId] = {
      ...existing,
    ...
    user.contactPrivacyOverrides = overrides;
    user.markModified('contactPrivacyOverrides');
    await user.save();

    res.status(200).json({ success: true, overrides: user.contactPrivacyOverrides });
  } catch (error) {
    console.error('setContactPrivacyOverride error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// GET /api/genz-mods/contact-privacy — list all per-contact overrides for the current user
exports.getContactPrivacyOverrides = async (req, res) => {
  try {
    const user = await getUser(req, res);
    if (!user) return;
    res.status(200).json({ success: true, overrides: user.contactPrivacyOverrides || {} });
  } catch (error) {
    console.error('getContactPrivacyOverrides error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// DELETE /api/genz-mods/contact-privacy/:contactId — reset a contact back to the global default
exports.clearContactPrivacyOverride = async (req, res) => {
  try {
    const user = await getUser(req, res);
    if (!user) return;
    const overrides = { ...(user.contactPrivacyOverrides || {}) };
    delete overrides[req.params.contactId];
    user.contactPrivacyOverrides = overrides;
    user.markModified('contactPrivacyOverrides');
    await user.save();
    res.status(200).json({ success: true, overrides: user.contactPrivacyOverrides });
  } catch (error) {
    console.error('clearContactPrivacyOverride error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
