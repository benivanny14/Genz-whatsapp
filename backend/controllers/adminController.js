...
exports.bootstrapAdmin = async (req, res) => {
  try {
    ...
    const existingAdmin = await AdminOwner.findOne({ ownerKey: 'PRIMARY_OWNER' });
    if (existingAdmin) {
    ...
    const totpSecret = speakeasy.generateSecret({ name: 'GENZ Admin', issuer: 'GENZ WhatsApp' }).base32;

    const admin = new AdminOwner({
      ownerKey: 'PRIMARY_OWNER',
      username,
      totpSecret,
      totpEnabled: false // Disable 2FA by default
    });
    ...
    return res.status(200).json({
      ...
      otpauth_url: `otpauth://totp/GENZ%20Admin:${username}?secret=${admin.totpSecret}&issuer=GENZ%20WhatsApp`
    });
  } catch (error) {
