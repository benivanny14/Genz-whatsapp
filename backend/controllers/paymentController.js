...
const VALID_PAYMENT_METHODS = ['mpesa', 'airtel', 'airtel-money', 'yas', 'halopesa', 'card', 'mock', 'manual'];
...
const isMockPaymentAllowed = () => (
  // SECURITY FIX: this used to default to "mock payments ON" unless someone
  // explicitly set DISABLE_MOCK_PAYMENTS=true. Since a real deployment
  // almost never has genuine M-Pesa/Airtel/Yas/HaloPesa merchant API keys
  // configured on day one (those require registering a business account
  // with each provider), the old default meant every "Pay Tsh 10,000" tap
  // silently completed for free with no money ever moving — anyone could
  // unlock Premium at no cost. Mock payments must now be turned on
  // on purpose (useful for local development/testing only).
...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    // SECURITY FIX: mock mode used to also auto-activate any time
    // NODE_ENV wasn't exactly 'production' — easy to forget to set on a
    // host like Render, which would have meant real users on a real
    // deployment got Premium for free. Mock mode now only applies when the
    // method is explicitly 'mock', and only if mock payments are allowed.
    let shouldUseMockPayment = normalizedMethod === 'mock';
    let shouldUseManualPayment = normalizedMethod === 'manual';
    ...
    // Manual payments don't require API keys - they're for admin/manual activation
    if (!shouldUseMockPayment && !shouldUseManualPayment) {
      const missingConfig = getMissingProviderConfig(normalizedMethod);
      if (missingConfig.length > 0) {
        // Do NOT fall back to mock/free-completion here — that would grant
        // Premium without any real payment. Block instead, with a message
        // that tells the operator exactly what's missing.
        console.error(`[Payment] Cannot process ${normalizedMethod}: missing ${missingConfig.join(', ')}`);
        return res.status(503).json({
          success: false,
          error: `Njia hii ya malipo (${normalizedMethod}) haijawekwa sawa na msimamizi wa mfumo bado. Tafadhali jaribu njia nyingine au wasiliana na msaada.`
        });
      }
    }
    ...
    if (shouldUseMockPayment || shouldUseManualPayment) {
      const subscription = await activateSubscription({
        ...
        transactionId,
        gatewayResponse: {
          provider: shouldUseManualPayment ? 'manual' : 'mock',
      ...
      return res.status(200).json({
        success: true,
        paymentUrl: shouldUseManualPayment ? `https://manual-payment.local/pay/${transactionId}` : `https://mock-payment-gateway.local/pay/${transactionId}`,
        transactionId,
        paymentStatus: 'completed',
        message: shouldUseManualPayment ? 'Payment completed in manual mode' : 'Payment completed in mock mode',
        subscription
      });
    }
