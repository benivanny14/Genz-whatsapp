...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (messageId) {
      const existingMessage = await Message.findOne({ clientMessageId: String(messageId), sender: localUserId })
        .populate("sender", "username profilePicture")
        .populate({
          path: "replyTo",
          select: "_id content messageType sender",
      ...
      if (existingMessage) {
        console.log(`[ChatController] Deduped message via API: ${messageId}`);
        return res.status(200).json({ success: true, message: existingMessage });
      }
    }
...
// Set/clear the translation language override for one specific chat (TM "Language Per Chat")
exports.setConversationLanguage = async (req, res) => {
  try {
    const userId = getCurrentUserId(req);
    const { conversationId } = req.params;
    ...
    res.json({ success: true, language: conversation.language });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
