...
const userSchema = new mongoose.Schema({
  ...
  // In-app permission flags granted by the admin (Roles & Permissions
  // section). Unrelated to admin-panel access, which is owner-only and
  // lives entirely in the separate AdminOwner collection.
  appPermissions: [{
    type: String,
    enum: ['moderate_groups', 'moderate_channels', 'view_reports', 'verified_badge']
  }],
...
userSchema.methods.setPassword = async function(password) {
  ...
  const N = 4096; // Lower cost for faster login/registration on low-spec servers
  const derivedKey = await scrypt(password, salt, PASSWORD_KEY_LENGTH, { N });
  this.passwordHash = `${salt}:${derivedKey.toString('hex')}:${N}`;
...
userSchema.methods.comparePassword = async function(password) {
  ...
  let salt, storedHash, N;
  
  if (parts.length === 2) {
    // Old format (uses default Node.js N=16384)
    [salt, storedHash] = parts;
    N = 16384; 
  } else if (parts.length === 3) {
    // New format with explicit N
    [salt, storedHash, N] = parts;
    N = parseInt(N, 10);
  } else {
    return false;
  }
