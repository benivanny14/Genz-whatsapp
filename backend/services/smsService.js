...
class SMSService {
  constructor() {
    ...
    if (!hasAfricasTalkingCreds() && !hasTwilioCreds()) {
      console.warn('[SMS] No SMS provider credentials configured — running in DEV MODE. Real text messages will NOT be sent; OTPs are only logged to the console. Fill in AFRICAS_TALKING_API_KEY/AFRICAS_TALKING_USERNAME or TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN/TWILIO_PHONE_NUMBER in your .env to send real SMS.');
    }
  }
   ...
   * configured (Africa's Talking preferred, then Twilio); otherwise falls
   * back to console logging so local development keeps working without a
   * paid SMS account.
   */
